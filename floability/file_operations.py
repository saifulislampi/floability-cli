...
def extract_tar(source_path: Path, params: Dict[str, Any] = None) -> Path:
    ...
    - params: Optional parameters dict with:
        - extract_dir: Directory to extract to (default: same name as tar without extension)
        - overwrite: Whether to overwrite existing files (default: False)
    ...
    if not source_path.is_file() or not (source_path.suffix.lower() == '.tar' or 
                                        source_path.name.lower().endswith(('.tar.gz', '.tar.bz2', '.tar.xz'))):
...
def execute_operation(operation_name: str, source_path: Path, params: Dict[str, Any] = None) -> Optional[Path]:
    """
